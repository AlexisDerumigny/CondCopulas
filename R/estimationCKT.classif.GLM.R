...
#'
#' This function fits a regression model for the
...
#' \deqn{\tau_{1,2|Z} = 2 \times \Lambda( \beta_0 + \beta_1 \phi_1(Z) + ... + \beta_p \phi_p(Z) )}
...
#' It should hve the same number of rows as the \code{datasetPairs}.
#'
...
#'
#' @return the fitted GLM, an object with S3 class "ordinalNet".
#'
#'
#' @references
...
#'
#' @param fit result of a call to CKT.fit.GLM
...
#' @export
#'
CKT.predict.GLM <- function(fit, newData){

  prediction = 2*stats::predict(fit, newx = newData, type = "response" )[,1] - 1
