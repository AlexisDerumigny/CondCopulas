...
We simulate a 3 dimensional dataset with one conditioning variable $X_3$, following a normal distribution.
Conditionally to $X_3 = x_3$, we simulate the two variables $X_1$ and $X_2$ as follows :
...
```
It seems that on the first plot, the dependence is quite high between $X_1$ and $X_2$, while it is much smaller in the second plot. This will be confirmed in the next section.
...
After the estimation, we can then compare the estimated conditional arameters with the true conditional parameter of the Clayton copula.
...
ggplot(data = comparison, aes(x = x3, y = param, color = as.factor(id), group = as.factor(id))) +
  geom_line() +
